...
{
  public class RawMaterialFabricController : Controller
  {
    private readonly IRawMaterialFabricService _service;
    private const int CategoryId = 37;
    private const int PageSize = 9;

    public RawMaterialFabricController(IRawMaterialFabricService service)
    {
      _service = service;
    }

    public async Task<IActionResult> ListFabric(int page = 1)
    {
      var products = await _service.GetProducts(CategoryId, page, PageSize);
      ViewData["SearchTerm"] = null;
      return View("~/Views/ProductQC/Raw Material Standard/ListFabric.cshtml", products);
    }

    public async Task<IActionResult> Search(string name, int page = 1)
    {
      if (string.IsNullOrEmpty(name))
        return RedirectToAction(nameof(ListFabric));
    ...
    public async Task<IActionResult> CreateFabric()
    {
      ...
      return View("~/Views/ProductQC/Raw Material Standard/CreateFabric.cshtml");
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> CreateFabric(RawMaterialFabricDTO product)
    {
      if (!ModelState.IsValid)
      {
        var categories = await _service.GetCategories();
        ViewBag.CategoryList = new SelectList(categories.Where(c => c.CategoryId == CategoryId), "CategoryId", "CategoryName");
        return View("~/Views/ProductQC/Raw Material Standard/CreateFabric.cshtml", product);
      }
    ...
    public async Task<IActionResult> EditFabric(int id)
    {
      ...
      var categories = await _service.GetCategories();
      ViewBag.CategoryList = new SelectList(categories.Where(c => c.CategoryId == CategoryId), "CategoryId", "CategoryName");
      return View("~/Views/ProductQC/Raw Material Standard/EditFabric.cshtml", product);
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> EditFabric(RawMaterialFabricDTO product)
    {
      if (!ModelState.IsValid)
      {
        var categories = await _service.GetCategories();
    ...
    public async Task<IActionResult> Delete(int productId)
    {
      ...
      return Json(new { success = true, message = "Fabric đã được xóa!" });
    }

    public async Task<IActionResult> ShowFabric(int id)
    {
      var product = await _service.GetProductByIdAsync(id);
      if (product == null) return NotFound();
      return PartialView("~/Views/ProductQC/Raw Material Standard/ShowFabric.cshtml", product);
    }

    private async Task HandleImageUpload(RawMaterialFabricDTO product)
    {
